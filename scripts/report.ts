...
function envLine(env: Env): string {
  const machine = cpus()[0]?.model ?? 'unknown CPU';
  const when = new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' });
  return (
    `> ${env.postgres}, Bun ${Bun.version}, ${machine}, ${when}. µs per operation, median of ` +
...
function resultsJs(results: Results, env: Env): string {
  const payload = {
    ...
    /** Relative half-width of each median's 95% interval, for error bars on the totals. */
